...
- **Arbitrage tranché (2026-08-04) : VRAIE DIMENSION SÉPARÉE.** L'écart technique de 2026-07-21 — une poche compacte vers x/z ≈ 20 000, adoptée parce qu'un placement naïf à `cellule × N` faisait trembler la caméra (la précision float32 s'effondre vers 4 000 000) — **n'existe plus**. L'intérieur vit dans la dimension `donjon`, en coordonnées LOCALES près de l'origine : un donjon à 256 blocs de l'origine ouvre sur un étage en (51, 4, 24). C'est la seconde issue qui était envisagée ici, et elle lève du même coup le problème de précision, quelle que soit la distance de la cellule. Verrouillé par `--probe-dimensions`, qui échoue si un étage repart aux coordonnées de sa cellule. Depuis le 2026-08-03, `DimensionManager` généralise le mécanisme : une dimension est une entrée de registre en données, le donjon n'est plus un cas du moteur mais un backend. **Reste ouvert** : les dimensions ne sont pas supportées en multijoueur (le routage RPC est overworld-only) — c'est cela, et non plus le placement, qu'il faut trancher avant le réseau.
...
- **Tous posés droit et à plat**, dans la même orientation : c'est la seule façon qu'une collection d'objets au sol se lise. **Clic droit à main vide** sur un objet posé le fait pivoter d'un **quart de tour** — l'orientation est un geste voulu, pas un tirage à la pose, et elle reste alignée sur la grille (4.1).
