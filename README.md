...
| Wu Xing (5.2, A.4.6) | **Fait le 2026-08-10** (amendement du 2026-08-09). Cinq éléments, deux cycles, dans [wu_xing.gd](systems/combat/wu_xing.gd) — LA table unique. Multiplicateur de domination en étape 3 du pipeline (`resolve_hit`), appliqué aux projectiles de modules (élément à l'impact contre la cible réelle) et à la mêlée des créatures (contre l'armure du joueur, `wu_element`). Combos d'engendrement (30 ticks, +1 dé, +50 % durée de statut), coût de mana modulé par le biome (affiché = débité). Alignements DÉRIVÉS partout (domaines de grimoire, tags de créature, catégorie d'armure, id de biome), la donnée explicite primant. Sonde `--probe-wuxing` (30 vérifications). *Restent ouverts : 5 modules Métal, affinités de cuisine, tooltip/pentagramme d'UI, élément des zones persistantes, mêlée joueur (attend le craft compositionnel).* |
